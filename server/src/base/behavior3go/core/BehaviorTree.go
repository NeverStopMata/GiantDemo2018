	...
	for id, s := range data.Nodes {
		...
		if extMaps != nil && extMaps.CheckElem(spec.Name) {
			...
			if tnode, err := extMaps.New(spec.Name); err == nil {
				...
				panic("new " + spec.Name + " err:" + err.Error())
			}
		} else {
			if tnode, err2 := maps.New(spec.Name); err2 == nil {
				node = tnode.(IBaseNode)
			} else {
				panic("new " + spec.Name + " err:" + err2.Error())
			}
		}
...
func (this *BehaviorTree) Tick(target interface{}, blackboard *Blackboard) b3.Status {
	...
	currOpenNodes := make(map[string]IBaseNode)
	for _, v := range tick._openNodes {
		currOpenNodes[fmt.Sprintf("%v", unsafe.Pointer(v.(*BaseNode)))] = v
	}
	...
	// close the nodes
	for _, v := range lastOpenNodes {
		if _, ok := currOpenNodes[fmt.Sprintf("%v", unsafe.Pointer(v.(*BaseNode)))]; !ok {
			v._close(tick)
		}
	}

	/* POPULATE BLACKBOARD */
	blackboard._getTreeData(this.id).OpenNodes = make([]IBaseNode, len(tick._openNodes))
	copy(blackboard._getTreeData(this.id).OpenNodes, tick._openNodes)
	blackboard.SetTree("nodeCount", tick._nodeCount, this.id)

	return state
}

func (this *BehaviorTree) Close(target interface{}, blackboard *Blackboard) {
	var tick = NewTick()
	tick.debug = this.debug
	tick.target = target
	tick.Blackboard = blackboard
	...
	blackboard._getTreeData(this.id).OpenNodes = make([]IBaseNode, 0)
