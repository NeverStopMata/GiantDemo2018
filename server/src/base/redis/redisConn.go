...
func exceptionProcess(redisCon *RedisCon) {
	for {
		...
		if !redisCon.cycleIsAlive() && redisCon.getProcessingCmdNum() <= 0 { //生命周期结束
			...
			glog.Error("[Redis] 手动关系redis服务,", redisCon.server)
			break
		}
...
func NewRedisCon(server string, passwd string, cmdChanCap int, waitChanCap int) (redisCon *RedisCon, err error) {
	svr, pwd, _ := ParseRedisKey(server)
	if svr == "" {
		glog.Error("[Redis] Invalid redis server address:", svr)
	...
	if len(pwd) > 0 {
		ch := make(chan *Result)
		redisCon.SendToConn(ch, "AUTH", pwd)
		result := <-ch
...
func (c *RedisCon) sendToCmdChan(ch chan *Result, cmd *string, argNum int, arg1 interface{}, arg2 interface{}, arg3 interface{}, args *[]interface{}) error {
	var sCmd *Command
	defer func() {
	...
	//队列满了则返回
	//if len(c.cmdChan) == cap(c.cmdChan){
	//    err = ErrChanBusy
	//    return
	//}
...
func (c *RedisCon) resetNet() error {
	...
	c.conRetriedTimes += 1
	netDialer := net.Dialer{Timeout: 0}
